...
import { ReviewUserIdProductIdCompoundUniqueInputObjectSchema as ReviewUserIdProductIdCompoundUniqueInputObjectSchema } from './ReviewUserIdProductIdCompoundUniqueInput.schema'

const makeSchema = () => z.object({
  id: z.string().optional(),
  userId_productId: z.lazy(() => ReviewUserIdProductIdCompoundUniqueInputObjectSchema).optional()
}).strict().superRefine((obj, ctx) => {
        const presentTop = (k: string) => (obj as any)[k] != null;
        const singles: string[] = ["id"] as string[];
        const groups: string[][] = [["userId","productId"]] as string[][];
